...
def _fit_into_field(vertices: list) -> list:
    """Масштабирует треугольник до поля + размещает его по центру."""
...
def process_request(side_a_str, side_b_str, side_c_str) -> dict:
    ...
    Значения, возвращаемые при ошибках ввода:
    * нечисловые данные            -> тип "" и координаты (-2, -2)
    * числовые данные, но не треугольник -> тип "не треугольник" и (-1, -1)
    """
    try:
        ...
        logging.error(
            "Неуспешный запрос: стороны '%s', '%s', '%s' "
    ...
    if not is_valid_triangle(*sides):
        logging.error(
            "Неуспешный запрос: стороны %s не образуют треугольник",
            sides,
        )
        return {
            "type": "не треугольник",
